import allure
from urls import PROFILE_URL, MAIN_PAGE_URL, ORDER_HISTORY_URL
...
class TestProfile:
    ...
    def test_go_to_order_history(self, main_page, profile_page):
        ...
        main_page.wait_for_url_contains(PROFILE_URL, timeout=25)
        
        # Переходим в историю заказов
        profile_page.click_order_history_link()
        
        # Увеличиваем время ожидания для перехода
        main_page.wait_for_url_contains(ORDER_HISTORY_URL, timeout=25)
    ...
    def test_logout(self, main_page, profile_page):
        ...
        # Увеличиваем время ожидания для перехода
        main_page.wait_for_url_contains(MAIN_PAGE_URL, timeout=30)
